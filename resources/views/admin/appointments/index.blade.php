...
@section('content')
    <div class="flex items-center justify-between mb-6">
        <div>
            ...
            <p class="text-sm text-slate-600">Gestiona estados y reprogramaciones de turnos.</p>
        </div>
        ...
        <table class="w-full text-sm">
            <thead class="bg-slate-50 text-slate-600">
                <tr>
                    ...
                    <th class="text-left px-4 py-3">Categoría / Servicio</th>
            ...
            <tbody class="divide-y divide-slate-100">
                @forelse($appointments as $appointment)
                    <tr>
                        ...
                        <td class="px-4 py-3">
                            <p class="font-medium text-slate-900">{{ $appointment->typeLabel() }}</p>
                            <p class="text-xs text-slate-500">{{ $appointment->serviceLabel() }}</p>
                        </td>
                        <td class="px-4 py-3">
                            <span class="rounded-full px-3 py-1 text-xs font-semibold
                                @if($appointment->status === 'confirmed') bg-emerald-100 text-emerald-800
                                ...
                                <div class="flex flex-wrap gap-2">
                                    <form method="POST" action="{{ route('admin.appointments.status', $appointment) }}">
                                        ...
                                        @if($appointment->status === 'cancelled')
                                            ...
                                            <button class="rounded-lg border border-emerald-300 px-3 py-1.5 text-xs font-semibold text-emerald-700">Reactivar</button>
                                        @else
